...
module.exports = {
  ...
  startInstallerUpdate() {
    ...
    .then(()=>{
      platform.startProcess(platform.getInstallerPath(), ["--unattended", "--update", "--path", path.join(platform.getTempDir(), config.getComponentInfo("InstallerElectron").extractTo)]);
    });
  },
